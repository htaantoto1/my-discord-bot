...
app = Flask(__name__)
# مفتاح عشوائي للجلسات (Session) عشان يحفظ بيانات المستخدم المؤقتة
...
def home():
    ...
    # إذا لم يقم المستخدم بتسجيل الدخول بعد، اعرض له زر تسجيل الدخول
    if not code:
        discord_login_url = (
        ...
        return f'''
            <div style="text-align: center; margin-top: 100px; font-family: Tahoma; background-color: #1e1e1e; color: white; padding: 40px; border-radius: 10px; width: 400px; margin-left: auto; margin-right: auto;">
                ...
                <p>اضغط الزر أدناه لعرض معلومات حسابك وسيرفراتك:</p>
    ...
    # إذا وافق المستخدم، نقوم بتبادل الكود بـ Access Token
    data = {
        'client_id': CLIENT_ID,
    ...
    # جلب معلومات المستخدم الأساسية (الاسم، الصورة، البنر)
    ...
    if isinstance(guilds_data, list):
        for guild in guilds_data:
            ...
            g_id = guild.get('id')
            if g_icon:
                icon_url = f"https://cdn.discordapp.com/icons/{g_id}/{g_icon}.png"
            else:
        ...
        guilds_html = "<p>تعذر جلب السيرفرات.</p>"

    # تصميم الصفحة النهائية بالموقع
    return f'''
        <div style="font-family: Tahoma; background-color: #36393f; color: white; padding: 30px; max-width: 600px; margin: auto; border-radius: 10px;">
            <h2 style="text-align: center;">لوحة معلومات الحساب</h2>
